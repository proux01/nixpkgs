...
let
  ...
  rocqlib-flags =
      [
        "COQLIBINSTALL=$(out)/lib/coq/${rocq.rocq-version}/user-contrib"
        "COQPLUGININSTALL=$(OCAMLFIND_DESTDIR)"
      ];
  docdir-flags =
      [ "COQDOCINSTALL=$(out)/share/coq/${rocq.rocq-version}/user-contrib" ];
in
  ...
  removeAttrs (
    {
      ...
      nativeBuildInputs =
        args.overrideNativeBuildInputs or (
          [ which ]
          ++ optional useDune rocq.ocamlPackages.dune_3
          ++ optionals (useDune || mlPlugin) [
            rocq.ocamlPackages.ocaml
            rocq.ocamlPackages.findlib
          ]
          ++ (args.nativeBuildInputs or [ ])
          ++ extraNativeBuildInputs
        );
      buildInputs =
        args.overrideBuildInputs or ([ rocq ] ++ (args.buildInputs or [ ]) ++ extraBuildInputs);
      inherit enableParallelBuilding;

      meta =
        (
          {
            platforms = rocq.meta.platforms;
          }
          // (switch domain [
            {
    ...
    // (optionalAttrs setROCQBIN { ROCQBIN = "${rocq}/bin/"; })
    ...
    // (optionalAttrs (args ? useMelquiondRemake) rec {
      COQUSERCONTRIB = "$out/lib/coq/${rocq.rocq-version}/user-contrib";
